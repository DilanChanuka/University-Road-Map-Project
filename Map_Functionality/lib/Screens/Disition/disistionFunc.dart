...
void disitionFunc(BuildContext context, GlobalKey _loader,List<String> arr)
{
    //0=>start place name
 ...
 Future<void> _loadingOuterRoutes(BuildContext context,GlobalKey _loader,String response,List<String> arrRoute) async{
    try{
 ...
 Future<void> _loadingGetPlace(BuildContext context,GlobalKey _loader,List<dynamic> response) async{
    ...
    try{
      ...
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) =>DrawPlace(drawplace(response[2]),response[0],response[1])));
    }
    catch(error){
      print(error);
    }
